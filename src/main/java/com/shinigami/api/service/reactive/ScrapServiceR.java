...
//    public Mono<BrowseModel> scrapBrowse() {
//        return Mono.fromCallable(new Callable<BrowseModel>() {
//            @Override
//            public BrowseModel call() throws Exception {
...
//    public Mono<BrowseModel> scrapBrowseV2(){
//        return Mono.fromCallable(new Callable<Document>() {
//            @Override
//            public Document call() throws Exception {
//                return Jsoup.connect("https://shinigami.id/")
...
//                        .get();
//            }
//        }).map(new Function<Document, BrowseModel>() {
//            @Override
//            public BrowseModel apply(Document document) {
//                List<ComicModel> all = scrapHome(document);
//
//                List<ComicModel> hotList = all.size() >= 8 ? all.stream().limit(8).toList() : all;
//                List<ComicModel> newsList = all.size() >= 15 ? all.stream().skip(8).limit(15).toList() : List.of();
//
//                List<ComicModel> trendingList;
//                try {
//                    trendingList = scrapBy(TRENDING, 1, false);
//                } catch (IOException e) {
...
//                return Mono.just(new BrowseModel(new ArrayList<>(), new ArrayList<>(), new ArrayList<>()));
//            }
//        }).subscribeOn(Schedulers.boundedElastic()); // pakai yg boundedElastic() instead parallel(), karena Schedulers.parallel() lebih di desain untuk blocking tasks
//    }
    ...
    public Mono<List<ComicModel>> scrapBy(String by, int page, boolean isMultiple) {
        try {
            ...
            return Mono.just(scrapBy(url, by, page, isMultiple));
        }catch (IOException e){
            throw new RuntimeException(e);
        }
    }
    ...
    public List<ComicModel> scrapBy(String url, String by, int page, boolean isMultiple) throws IOException {
        ...
        if (isMultiple && count < 1){
            ...
            comicList.addAll(scrapBy(by, page + 1, isMultiple).block());
        }else{
            count = 0;
        }
